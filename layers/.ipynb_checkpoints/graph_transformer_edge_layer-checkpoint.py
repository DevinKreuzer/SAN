...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, Lambda, in_dim, out_dim, num_heads, full_graph, use_bias):
        super().__init__()
    ...
    def propagate_attention(self, g):
        ...
        if self.full_graph:
            real_ids = torch.nonzero(g.edata['feat']).view(-1)
            fake_ids = (g.edata['feat']==0).nonzero().view(-1)
...
class GraphTransformerLayer(nn.Module):
    """
        Param: 
    """
    def __init__(self, Lambda, in_dim, out_dim, num_heads, full_graph, dropout=0.0, layer_norm=False, batch_norm=True, residual=True, use_bias=False):
        super().__init__()
        ...
        self.attention = MultiHeadAttentionLayer(Lambda, in_dim, out_dim//num_heads, num_heads, full_graph, use_bias)
